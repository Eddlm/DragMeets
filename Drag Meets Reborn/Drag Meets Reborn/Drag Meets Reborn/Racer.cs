...
namespace Drag_Meets_Reborn
{
    public class Racer
    {
        ...
        public Racer(string name, Ped ped, Vehicle veh, int customTurbo, int reaction, int shifting, bool notify, int won, int lost, int money)
        {
            ...
            if (Car.ClassType == VehicleClass.Super) UsesParachute = true;
            if (!Driver.IsPlayer)
            {
                Wins = won;
                Loses = lost;
                ...
                if (Car.Position.DistanceTo(DragMeets.Finishline) < Car.Position.DistanceTo(DragMeets.StartingLine))
                    TaskToWaitingArea();
            }
            else
        ...
        {
            if (Car.IsInRangeOf(Util.ToGround(Util.GetEntityOffset(DragMeets.FlarePed, DragMeets.ForwardOffset, Offset), 5f), 4f))
                return Car.Speed < 0.300000011920929;
            return false;
        }
                ...
                if (DragMeets.GetPlayerRacer() != null)
                {
                    if (!BetsOnHim.Contains(DragMeets.GetPlayerRacer()) && !DragMeets.GetPlayerRacer().HasBet)
                    {
                        if (DragMeets.GetPlayerRacer().Picked == RacerStatus.Picked)
                            ...
                            Util.DisplayHelpTextThisFrame("Press " + Util.GetInstructionalButton(DragMeets.InteractKey) + " to bet ~g~$" + DragMeets.MoneyBet + "~w~ on this guy.");
        ...
        {
            int num1 = 10;
            float num2 = 50f + Wins * num1 - Loses * num1;
            if (num2 < 0.0)
                num2 = 0.0f;
            if (num2 > 100.0)
                num2 = 100f;
        ...
        public void OnTick()
        {
            if (Util.CanWeUse(Parachute))
            {
                if (Util.ForwardSpeed(Car) > 4.0)
            ...
            if (DragMeets.GetPlayerRacer() != null && Picked == RacerStatus.NotPicked && (!Forced && !Driver.IsPlayer) && Util.EntitySeenByPlayer() == Car)
            {
                ...
                if (Game.IsControlJustPressed(2, DragMeets.InteractKey))
                {
                    ...
                    if (DragMeets.FairChallenges)
                    {
                        ...
                        {
                            if (PerformancePoints > DragMeets.GetPlayerRacer().PerformancePoints)
                            {
                                if (ReputationBalance() > DragMeets.GetPlayerRacer().ReputationBalance())
                                {
                            ...
                            else if (ReputationBalance() < 60.0)
                            {
                        ...
                        else if (System.Math.Abs(ReputationBalance() - DragMeets.GetPlayerRacer().ReputationBalance()) < 10.0)
                        {
                            text = "~b~" + Name + "~w~ thinks this will be a close race.";
                        }
                        else
                        {
                            if (ReputationBalance() > DragMeets.GetPlayerRacer().ReputationBalance() + 20.0)
                                text = "~b~" + Name + "~w~ expects you to lose.";
                            if (DragMeets.GetPlayerRacer().ReputationBalance() > ReputationBalance() + 20.0)
            ...
            if (Game.Player.Character.IsOnFoot && Car == Util.EntitySeenByPlayer())
            {
           ...
           Game.Player.Character
                ...
                {
                    float num1 = (float)System.Math.Round(Function.Call<float>(Hash._0xF417C2502FFFED43, new InputArgument[1]
                    {
             Car.Model.Hash
                    }) * 2.20000004768372);
                    float num2 = (float)System.Math.Round(Function.Call<float>(Hash._0xAD7E85FC227197C4, new InputArgument[1]
                    {
             Car
                    }), 2);
                    float num3 = (float)System.Math.Round(Function.Call<float>(Hash._0x8C044C5C84505B6A, new InputArgument[1]
                    {
             Car.Model
                    }), 2);
                    ...
                    if (num3 > 100.0)
                        num3 = 99.9f;
                    racertext.CallFunction("SET_VEHICLE_INFOR_AND_STATS", (Name + " ($" + Driver.Money + ") - " + Car.FriendlyName), ("Wins/Loses:  " + Wins + "/" + Loses), "MPCarHUD", "Dinka", ("Wheel Temp: " + WheelTemp + "º"), ("Speed: " + str + " mph"), "Acceleration", "Braking", WheelTemp, num1, (float)(num3 * 100.0), (float)(num2 * 100.0));
                ...
                {
                    float num1 = (float)System.Math.Round(Shifting - Reaction * 0.00999999977648258);
                    if (num1 > 100.0)
                        num1 = 100f;
                    else if (num1 < 0.0)
                        num1 = 0.0f;
                    float num2 = (Car.GetMod(VehicleMod.Engine) + Car.GetMod(VehicleMod.Transmission)) * 10;
                    if (num2 < 0.0)
                        num2 = 0.0f;
                    racertext.CallFunction("SET_VEHICLE_INFOR_AND_STATS", Name, Car.FriendlyName, "MPCarHUD", "Dinka", "Reputation", "Skill", "Performance ", "Upgrades", ReputationBalance(), num1, PerformancePoints, num2);
                    racertext.Render3D(Car.Position + new Vector3(0.0f, 0.0f, Car.Model.GetDimensions().Z + 1f), GameplayCamera.Rotation, new Vector3(6f, 3f, 1f));
                }
            }
            switch (DragMeets.MeetState)
            {
                ...
                case DragRaceState.Setup:
                    ...
                    if (!Driver.IsPlayer || Picked != RacerStatus.Picked)
                        break;
                    string str1 = "";
                    if (WheelTemp < DragMeets.IdealWheelTemp)
                        str1 = "~b~";
                    if (WheelTemp > DragMeets.IdealWheelTemp)
                        str1 = "~g~";
                    if (WheelTemp > (DragMeets.IdealWheelTemp + 20))
                        str1 = "~y~";
                    if (WheelTemp > (DragMeets.IdealWheelTemp + 30))
                        str1 = "~o~";
                    if (WheelTemp > (DragMeets.IdealWheelTemp + 40))
                        str1 = "~r~";
                    UI.ShowSubtitle(str1 + "Wheel temp: " + WheelTemp + "º", 900);
                    break;
                ...
                case DragRaceState.RaceInProgress:
                    if (Picked != RacerStatus.Picked)
                        break;
                    if (Driver.IsPlayer && DragMeets.SimulateManualTransmission && (DragMeets.SpectCamera == null && !DragMeets.MT_Support))
                    {
                    ...
                    if (Wheelietime > Game.GameTime & Function.Call<bool>(Hash._0x5333F526F6AB19AA, Car, 10f) && Car.Speed > 0.2)
                    {
                        if (GripBonus >= PerformanceBonus || !DragMeets.RealisticWheelies)
                        {
                            if (Wheelietime > Game.GameTime + 5500)
                                Wheelietime = Game.GameTime + 5000;
                            float num = (float)(1.0 + Car.HeightAboveGround * 3.0);
                            if (num > 3.0)
                                num = 3f;
                    ...
                    if (NitroTime > Game.GameTime && Car.Acceleration > 0.0)
                        Util.ForceNitro(Car);
                    if (Driver.IsPlayer && DragMeets.SimulateManualTransmission && (!DragMeets.MT_Support && GearBoostTime < Game.GameTime))
                    {
                        ...
                        {
                            if (Driver.IsPlayer && DragMeets.SpectCamera == null)
                            ...
                            if (Game.IsControlJustPressed(0, DragMeets.ShiftingKey))
                            {
                                ...
                                if (DragMeets.ShowGoodBadShifting)
                                    UI.Notify("~b~" + Car.FriendlyName + " ~g~perfect shift to " + (GearReference + 1));
                        ...
                        else if (Game.GameTime - ShiftBoostRef < 700)
                        {
                            if (Game.IsControlJustPressed(0, DragMeets.ShiftingKey))
                            {
                                ...
                                if (num > 0)
                                    Car.EngineHealth -= num;
                                else
                                    num = 0;
                                string str2 = Car.EngineHealth >= 200.0 ? " - engine grind (damage:" + num + ")" : " - ~r~engine blowup";
                                if (DragMeets.ShowGoodBadShifting)
                                    UI.Notify("~b~" + Car.FriendlyName + " ~o~bad shift to " + (GearReference + 1) + str2);
                            }
                        }
                    ...
                    if (DragMeets.SimulateManualTransmission && Car.CurrentGear > 1 && (Car.CurrentGear > GearReference && Car.EngineHealth > 200.0))
                    {
                        ...
                        if (!Driver.IsPlayer)
                        {
                            ...
                            if (num1 < 25)
                            {
                                ...
                                if (num2 > 0)
                                    Car.EngineHealth -= num2;
                                else
                                    num2 = 0;
                                string str2 = Car.EngineHealth >= 200.0 ? " - engine grind (damage:" + num2 + ")" : " - ~r~engine blowup";
                                if (DragMeets.ShowGoodBadShifting)
                                    UI.Notify("~b~" + Car.FriendlyName + " ~o~bad shift to " + (GearReference + 1) + str2);
                            }
                        }
                    }
                    if (Car.EngineHealth <= 205.0 && Car.Speed > 20.0)
                    ...
                    else if (GearBoostTime > Game.GameTime && Car.Acceleration > 0.0)
                    {
        ...
        {
            if (Car.BodyHealth < 1000.0)
                Car.Repair();
            if (Driver.IsPlayer)
            {
                Destination = Util.ToGround(Util.GetEntityOffset(DragMeets.FlarePed, DragMeets.ForwardOffset, Offset), 5f);
            }
            else
            {
                ...
                if (GotToMidPoint || DragMeets.PathToStartingLine == Vector3.Zero)
                {
                    if (!DragMeets.FlarePedInPosition() || DragMeets.IsStartLineBusy)
                        return;
                    Vector3 ground = Util.ToGround(Util.GetEntityOffset(DragMeets.FlarePed, 0.0f, Offset), 5f);
                    Destination = Util.ToGround(Util.GetEntityOffset(DragMeets.FlarePed, 15f, Offset), 5f);
                    if (!Car.IsInRangeOf(ground, 30f))
                    {
                        if (Driver.IsPlayer)
                            return;
                        float speed = Util.RandomInt(5, 8) * DragMeets.SpeedMultiplier;
                    ...
                    else
                    {
                        ...
                        if (!Driver.IsInRangeOf(Destination, 2f) && (Picked == RacerStatus.Picked || DragMeets.MeetState == DragRaceState.RaceInProgress))
                        {
                            if (Driver.IsPlayer || DragMeets.MeetState != DragRaceState.Setup)
                                return;
                            if (Util.IsPosAheadentity(Car, Destination, 0.0f))
                                ...
                                Driver.Task.DriveTo(Car, Util.ToGround(Util.GetEntityOffset(DragMeets.FlarePed, 20f, Offset), 5f), 5f, Util.RandomInt(2, 4) * DragMeets.SpeedMultiplier, DrivingStyle + 1024);
                        }
                        else
                        {
                            if (Util.IsDriving(Driver))
                            ...
                            else if (Car.GetMod(VehicleMod.Engine) > 1 & doburnouts && Util.ThrowDice(20 + (DragMeets.IdealWheelTemp - (int)WheelTemp)))
                            {
                                ...
                                if (WheelTemp + 10.0 <= DragMeets.IdealWheelTemp)
                                    return;
                ...
                else
                {
                    ...
                    {
                        float speed = Util.RandomInt(5, 10) * DragMeets.SpeedMultiplier;
        ...
        private void HandleWaitingAreaBehavior()
        {
           ...
           Car
            ...
            else if (Car.IsInRangeOf(DragMeets.WaitingArea, DragMeets.WaitingAreRange * 10f) && Car.Speed > 15.0)
            ...
            if (Car.IsInRangeOf(DragMeets.WaitingArea, DragMeets.WaitingAreRange))
            {
                ...
                {
                    Destination = Util.GetEntityOffset(Car, (int)System.Math.Round(DragMeets.WaitingAreRange * 0.5), Util.RandomInt(-10, 10));
                ...
                else
                {
                    ...
                    Driver.Task.GoTo(Util.GetEntityOffset(Car, 4f, 0.0f));
                }
            }
            else
            {
                ...
                {
                    Destination = DragMeets.WaitingArea.Around(DragMeets.WaitingAreRange / Util.RandomInt(1, 4));
                    if (!Util.CarCanSeePos(Car, Destination))
                        drivingstyle = 61;
                    Driver.Task.DriveTo(Car, Destination, 4f, Util.RandomInt(4, 8) * DragMeets.SpeedMultiplier, drivingstyle);
                }
                else
                {
                    if (!Util.CarCanSeePos(Car, DragMeets.WaitingArea))
                        drivingstyle = 61;
                    Driver.Task.DriveTo(Car, DragMeets.WaitingArea, DragMeets.WaitingAreRange + 10f, Util.RandomInt(20, 35) * DragMeets.SpeedMultiplier, drivingstyle);
                }
            }
        }
        ...
        {
            if (Driver.IsInVehicle(Car) && Car.Speed > 1.0)
            {
            ...
            else
            {
                if (Driver.IsPlayer)
                {
                    ...
                    else if (WheelTemp > (DragMeets.IdealWheelTemp - 15))
                        WheelTemp = WheelTemp - 0.1f;
                }
                else
                {
                    if (WheelTemp > (DragMeets.IdealWheelTemp - 15))
                        WheelTemp = WheelTemp - 0.1f;
                ...
                WheelTemp = (float)System.Math.Round(WheelTemp, 1);
                if (StuckTimeRef < Game.GameTime)
                {
                    ...
                    {
                        StuckScore = !Util.IsDriving(Driver) || !Util.CanWeUse(Car.GetPedOnSeat(VehicleSeat.Driver)) || (!Car.IsInRangeOf(StuckReference, 5f) || Driver.IsOnFoot) ? 0 : StuckScore + 1;
                ...
                if (StuckScore == 3)
                {
                    ...
                    Driver.Task.DriveTo(Car, Util.GetEntityOffset(Car, -9f, Util.RandomInt(-5, 5)), 5f, 4f, 4195388);
                ...
                if (StuckScore == 10)
                {
                    if (Picked == RacerStatus.Picked && DragMeets.PathToStartingLine != Vector3.Zero)
                        ...
                        Car.Position = Util.GetEntityOffset(DragMeets.FlarePed, 10f, 0.0f);
                    StuckScore = 0;
                ...
                switch (DragMeets.MeetState)
                {
                    ...
                    case DragRaceState.RaceInProgress:
                        ...
                        if (Picked != RacerStatus.Picked)
                            break;
                        if (!RacingToFinish && !Util.CanWeUse(Parachute) && (Util.ForwardSpeed(Car)>70 || UsesParachute))
                        {
                            Parachute = World.CreateProp("p_cargo_chute_s", Car.Position, false, false);
                            Parachute.AttachTo(Car, Util.GetBoneIndex(Car, "boot"), Vector3.Zero, new Vector3((float)(-Car.Rotation.X + 85.0), -Car.Rotation.Y, 0.0f));
                        }
                        if (!Driver.IsPlayer)
                        {
                        ...
                        if (Driver.IsPlayer || Util.IsDriving(Driver) || (RacingToFinish || Util.ForwardSpeed(Car) <= 5.0) || !Util.IsSliding(Car, 0.2f))
                            break;
        ...
        public void TaskFinish()
        {
            ...
            else if (RacesDone >= 2 && ReputationBalance() < 30.0)
            {
                RemoveMe = true;
        ...
        public void TaskRace()
        {
            ...
            if (Car.GetMod(VehicleMod.Engine) + Car.GetMod(VehicleMod.Transmission) > 0)
                PerformanceBonus = (Car.GetMod(VehicleMod.Engine) + Car.GetMod(VehicleMod.Transmission)) * 10;
            ...
            if (EngineBonusMultiplier < 1.0)
                EngineBonusMultiplier = 1f;
            string message = friendlyName + "~n~~b~Performance:" + PerformanceBonus + "%" + "~n~~b~Grip: " + GripBonus + "%" + "~n~~b~PowerMultiplier: " + (float)(EngineBonusMultiplier + 100.0) + "%";
            Car.EnginePowerMultiplier = EngineBonusMultiplier * 0.1f + CustomTurbo;
            if (DragMeets.RealisticWheelies)
            {
                if (!Util.CannotWheelie.Contains(Car.Model) && GripBonus - num2 >= num1 && (Car.GetMod(VehicleMod.Engine) > 1 && Car.GetMod(VehicleMod.Transmission) > 1))
                {
                    int num3 = (int)((WheelTemp - DragMeets.IdealWheelTemp) * 100.0);
                    if (num3 > 600)
                        Wheelietime = Game.GameTime + num3;
                }
            }
            else if (!Util.CannotWheelie.Contains(Car.Model) && GripBonus >= num1)
                Wheelietime = Game.GameTime + Util.RandomInt(2, 4) * 1000;
            Function.Call(Hash._0xFB8794444A7D60FB, Car, false);
            RacingToFinish = true;
            Destination = Util.ToGround(Util.GetEntityOffset(DragMeets.FinishBarrel, 10f, -Offset), 10f);
            if (Destination == Vector3.Zero)
                Destination = Util.GetEntityOffset(DragMeets.FinishBarrel, 10f, -Offset);
            if (!Driver.IsPlayer)
            {
                int num3 = 1;
                int num4 = 0;
                if (DragMeets.ReactionTimes)
                {
                    ...
                    if (DragMeets.ShowReactionTimes)
                        message = message + "~n~~w~React.Time: ~y~" + num4;
                }
                int num5 = 16777244;
        ...
        public void Clear(bool HardDeletion)
        {
            ...
            else if (Util.CanWeUse(Car) && Car.IsPersistent)
