...
namespace Drag_Meets_Reborn
{
    ...
    /// --- TO DO ---
    /// - 1.9.4:
    /// - Fix/Remove player persistence
    /// - Fix/Remove nontested meets
    /// - Release:
    /// - Lower wins/loses values at the Racers' panel
    /// - Option to force Self Radio on meets music - DONE
    /// - Crowd cheers
    ...
    public class DragMeets : Script
    {
        ...
        public static string ScriptVer = "1.9.3c";
...
"u_m_y_tattoo_01",
        ...
        public static bool DetailsPrompt = false;
        ...
        void OnTick(object sender, EventArgs e)
        {
            ...
            if (Util.WasCheatStringJustEntered("base bet"))
            {
                ...
                {
                    MoneyBet = temp;
                    UI.Notify("~g~New distance accepted.~n~~w~Any new bet will be $"+MoneyBet+".");
                }
            }
...
/*
                            ...
                            {
                                ChallengePrize = temp;
                                UI.Notify("The bet is now ~g~$" + ChallengePrize + "~w~.");
                            }
            ...
            if (Util.WasCheatStringJustEntered("cheat")) UI.Notify("~g~Cheat detected");

            if (Util.WasCheatStringJustEntered("leave meet") && GetPlayerRacer() != null) GetPlayerRacer().RemoveMe = true;
            if (Util.WasCheatStringJustEntered("despawn meet") && IsAnyMeetLoaded()) DespanwnRequested = true;
            ...
            if (IsAnyMeetLoaded())                
            {
                        ...
                        if (veh.IsInRangeOf(WaitingArea, WaitingAreRange))
                        {
                            ...
                            if (Game.IsControlJustPressed(2, InteractKey))
                            {
                                ...
                                if (CanRegister)
                                {
                                    ...
                                    if (VehicleListUpdater) SmartVehicleAddition(Game.Player.Character, veh);
                ...
                {
                    if (GetPlayerRacer().Car.IsInBurnout() && Game.IsControlJustPressed(2, GTA.Control.CharacterWheel))
                    {
                        GetPlayerRacer().RemoveMe = true;
                    }
                    Vehicle veh = Game.Player.Character.CurrentVehicle;
        ...
        public void SmartVehicleAddition(Ped ped, Vehicle veh)
        {
            bool Details = Game.IsControlPressed(2, GTA.Control.CharacterWheel);
            ...
            {
                WarnPlayer(ScriptName, "Vehicle already saved", "Racers.xml alredy contains 2 ~b~" + veh.FriendlyName + "s~w~.");
            }
            else
            {
                string Name = veh.FriendlyName;
                if (Name == "NULL") Name = veh.DisplayName;
                string CarKind = KindsAllowed[0];
                int Shift = 50;
                ...
                else
                {
                    ...
                    Util.SaveDriverToFile(Name, null, veh, RacersFile, false, false, KindsAllowed[0], Shift, ReactionTime);
                }
            }
        }
                    ...
                    if (Spawn)
                    {
                        ...
                        foreach (string meet in Directory.GetFiles(@"scripts\\DragMeets\Meets"))
                        {
                            if (File.Exists(meet))
                            {
                                number++;
                                filetrigger.Load(meet);
                                if (filetrigger == null) UI.Notify("~r~Failed to load" + meet);
                                else
                                {
                                    XmlNode list = filetrigger.SelectSingleNode("//Trigger");
                                    if (list == null) list = filetrigger.SelectSingleNode("//WaitingZone");
                                    if (list == null) UI.Notify("~r~Failed to load" + meet);
                                    else
                                    {
                                        ...
                                        if (meetPos == pos)
                                        {
                                           if(!immersive) Game.FadeScreenOut(500);
                                            ...
                                            else UI.Notify("~r~PathToStartingLine not found");
                                            ...
                                            */
                                            ...
                                            UI.Notify("~g~" + meet + " loaded.");


                                            //""
                                            if (StartingLineProp != "")
                                            {
                                                Prop prop = World.CreateProp((Model)StartingLineProp, Util.GetEntityOffset(FlarePed, -3, 0), false, true);
                                            ...
                                            if (WaitingZoneProp != "")
                                            {
                                                Prop prop = World.CreateProp((Model)WaitingZoneProp, WaitingArea, false, true);
                        ...
                        foreach (Racer winner in Winners)
                        {
                            ...
                            if (winner == Winners[0])
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        Util.Notify("CHAR_LAMAR", "~b~Lamar", winner.Name + " won", "~b~" + winner.Name + "~w~ won the " +ChallengePrize+ " bet! Now, for the other bets...");
       ...
       public static bool FlarePedInPosition()
        ...
        void LoadSettings(string file)
        {
            if (File.Exists(file))
            {
                ...
                DetailsPrompt = config.GetValue<bool>("VEHICLE_LIST_UPDATER", "DetailsPrompt", false);
...
*/
                    ...
                    {
                        WarnPlayer(ScriptName + " " + ScriptVer, "VEHICLE LOAD ERROR", "Error trying to load " + driver.SelectSingleNode("Vehicle/Model").InnerText + ". Skipping this car.");
                        File.AppendAllText(@"scripts\\DragMeets\debug.txt", " - " + " ERROR, SKIPPED");
